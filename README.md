...
## DNS doctor
Beginning in 2026, the `misc.txt` list is generated by a predictive algorithm known as *DNS Doctor* and predominantly includes domains that are candidates for future inclusion in `ads.txt` and `analytics.txt`.

In addition, non-existent or unresolvable domains are systematically excluded from all lists.
